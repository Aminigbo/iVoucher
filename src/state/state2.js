...
export const AppProvider = ({ children }) => {
    ...
    useEffect(() => {
        const loadProfile = () => {
            try {
                ...
                const storedCardInfo = storage.getString('CardInfo');
    ...
    const actions = useMemo(() => {
        ...
        const handleSaveCardDetails = (CardInfo) => {
            try {
                storage.set('CardInfo', JSON.stringify(CardInfo));
                setCardInfo(CardInfo);
            } catch (error) {
                console.error('Failed to card details:', error);
            }
        };
    ...
    const state = useMemo(() => ({
        ...
        AllUsers,
        CardInfo
    }), [User, Initialized, Transactions, AllBanks, isBiometric, Loading, loadingText, SelectedBank, AllUsers, CardInfo]);
